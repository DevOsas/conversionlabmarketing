...
function buildEmailHtml(data, results) {
  return `
    <div style="font-family: Arial, sans-serif; color: #1f2937; line-height: 1.6;">
      <div style="background: #0B0B0B; padding: 24px; color: white; border-radius: 16px 16px 0 0;">
        <h1 style="margin: 0; font-size: 24px;">Salesforge Digital Agency</h1>
        <p style="margin: 8px 0 0; color: #fecaca;">Customer Retention Revenue Report</p>
      </div>
      <div style="border: 1px solid #e5e7eb; border-top: 0; border-radius: 0 0 16px 16px; padding: 24px;">
        ...
        <p>Attached is your Salesforge Digital Agency revenue opportunity report for <strong>${sanitizeText(data.businessName)}</strong>.</p>
        ...
        <p><a href="${bookingLink}" style="color: #DC2626;">${bookingLink}</a></p>
        <p style="margin-top: 24px;">Salesforge Digital Agency</p>
      </div>
    </div>
  `;
}
...
function buildEmailText(data, results) {
  return [
    `Hi ${data.contactName},`,
    "",
    `Attached is your Salesforge Digital Agency Customer Retention Revenue Report for ${data.businessName}.`,
    ...
    "",
    "Salesforge Digital Agency"
  ].join("\n");
}

function buildLeadPayload(data, results) {
  return {
    source: "salesforge-retention-calculator",
...
function generatePdfBuffer(data, results) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor("#FFFFFF").fontSize(24).text("Salesforge Digital Agency", margin, 34);
    doc.fontSize(12).fillColor("#FECACA").text("Customer Retention Revenue Report", margin, 68);
    ...
    doc.fillColor("#FECACA").fontSize(10.5).text(bookingLink, margin + 18, currentY + 40, {
...
app.post("/api/calculate", async (req, res) => {
  try {
    ...
    const emailPayload = {
      to: cleanData.email,
      subject: `Your Salesforge Revenue Opportunity Report, ${cleanData.contactName}`,
      text: buildEmailText(cleanData, results),
      html: buildEmailHtml(cleanData, results),
      attachments: [
        {
          filename: "salesforge-revenue-report.pdf",
          content: pdfBuffer
        }
      ]
    };
...
function logStartupStatus() {
  ...
  console.log(`Salesforge app listening on http://localhost:${PORT}`);
